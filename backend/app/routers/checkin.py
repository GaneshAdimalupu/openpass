...
def scan_qr(
    ...
    reg = db.query(Registration).filter(Registration.qr_code == payload.qr_token).first()

    if not reg:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Invalid QR code — ticket not found"
        )

    # Make sure this event belongs to the scanning organizer
    event = (
        db.query(Event)
        .filter(Event.id == reg.event_id, Event.organizer_id == current_user.id)
        .first()
    )
    if not event:
        raise HTTPException(
    ...
    if reg.is_checked_in:
        last_checkin = (
            db.query(CheckIn)
            .filter(CheckIn.registration_id == reg.id)
